...
SELECT 
    ...
    AVG(CASE
        ...
        WHEN contract_type = 3 THEN (spot_price * (tax_percentage / 100. + 1.0)) - COALESCE(energy_margin, 0)
        ELSE NULL
    ...
    (AVG(CASE
        ...
        WHEN contract_type = 3 THEN (spot_price * (tax_percentage / 100. + 1.0)) - COALESCE(energy_margin, 0)
        ELSE NULL
    END) - COALESCE(AVG(transfer_fee), 0) - COALESCE(AVG(transfer_tax_fee), 0)) * SUM(value) AS price,
...
FROM 
    energies_production_hour_by_hour
GROUP BY 
